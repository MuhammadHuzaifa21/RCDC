...
public partial class Pages_RIS_Rec : System.Web.UI.Page
{
    ...
    void LoadRecord()
    {
        using (OracleConnection con = new OracleConnection(connStr))
        {
            ...
            // ✅ Check RIS Amount Charges Exists
            string checkQuery = "SELECT COUNT(*) FROM DCRC_RIS WHERE REG_NO = :regNo";
            ...
            using (OracleCommand cmdCheck = new OracleCommand(checkQuery, con))
            {
                ...
                if (count == 0)
                {
                    lblStatus.Text = "No RIS Charges Found!";
                    lblStatus.ForeColor = System.Drawing.Color.Orange;
                ...
                using (OracleTransaction trans = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        string updateDCRC = @"
                                UPDATE DCRC 
                                SET 
                                    RIS_AMT_REC = NVL(RIS_AMT_REC, 0) + :AMT,
