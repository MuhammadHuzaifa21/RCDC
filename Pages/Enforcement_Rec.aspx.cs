...
public partial class Pages_Enforcement_Rec : System.Web.UI.Page
{
    ...
    void LoadRecord()
    {
        using (OracleConnection con = new OracleConnection(connStr))
        {
            ...
            using (OracleCommand cmdCheck = new OracleCommand(checkQuery, con))
            {
                ...
                if (count == 0)
                {
                    lblStatus.Text = "No Enforcement Charges Found!";
                ...
                using (OracleTransaction trans = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        string updateDCRC = @"
                                UPDATE DCRC 
                                SET 
                                    ENFORCEMENT_AMT_REC = NVL(ENFORCEMENT_AMT_REC,0) + :AMT,
